...
export class EditorService {
        ...
        games: [
          {
            ...
            edition: 2009,
            mechanisms: [], // Doit être une liste de Mechanism[]
            editor: { 
              id: 1, 
              name: 'Asmodee', 
              exposant: true, 
  ...
  findByIdBD(id: number) {
    return this.http.get<Editor>(`https://localhost:4000/api/editeurs/${id}`)
  }
  
  addEditor(editor: Editor): void { this._editors.update(list => [...list, editor]) }

  addEditorBD(editor: Editor) {
  ...
  updateEditorBD(partial: Partial<Editor>, id: number) {
